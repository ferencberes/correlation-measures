...
def correl_var(n, s):
    ...
    avg = float(n+1) / 2    
    avg_weighted = pcc.avg_w(range(1,n+1))/s
    ret_val = 0.0
    ret_val_w = 0.0
    for i in range(n):
        w = 1.0/(i+1)
...
def compute_corr(list_a, list_b):
    ...
    n = len(list_a)
    h_n = pcc.szum(n)
    val, w_val = pcc.correl(list_a, list_b, h_n)
...
def corr(top_list_prev, top_list, sorted_id):
    ...
    #print list_a
    #print list_b
    return pcc.compute_corr(list_a, list_b)
...
def pre_proc(centrality_map):
    ...
    ret_val, ret_sort = pcc.pre_proc(centrality_map)
